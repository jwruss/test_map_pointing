 ...
 *      Author: John Russell
 *
 *  Produces files with histograms of the geometric time delays between baselines
 *  used in ANITA's interferometric maps.
 */
...
void makeDeltaT(TFile * outFile, AnitaPol::AnitaPol_t pol, int ant1, int ant2) {
	...
	TH2F deltaTMap(TString::Format("%s_%d_%d", pol == AnitaPol::kHorizontal ? "H" : "V", ant1, ant2), TString::Format("%s_%d_%d", pol == AnitaPol::kHorizontal ? "H" : "V", ant1, ant2), NPhi, minPhi, maxPhi, NNegTheta, minNegTheta, maxNegTheta);

	//  Fill the TH2F object.
	...
	//  Write filled TH2F object to file.
	outFile -> cd();
	deltaTMap.Write();
}
...
int main(int argc, char * argv[]) {
	...
	//  Create file in which coarsely binned TH2F objects will be written.
	TFile coarseFile("deltaTCoarse.root", "recreate");

	for (int i = 0; i < NUM_SEAVEYS; ++i) {

		for (int j = i + 1; j < NUM_SEAVEYS; ++j) {

			makeDeltaT(& coarseFile, AnitaPol::kHorizontal, i, j);
			makeDeltaT(& coarseFile, AnitaPol::kVertical, i, j);
		}
	...
	//  Create file in which finely binned TH2F objects will be written.
	TFile fineFile("deltaTFine.root", "recreate");

	for (int i = 0; i < NUM_SEAVEYS; ++i) {

		for (int j = i + 1; j < NUM_SEAVEYS; ++j) {

			makeDeltaT(& fineFile, AnitaPol::kHorizontal, i, j);
			makeDeltaT(& fineFile, AnitaPol::kVertical, i, j);
		}
	}

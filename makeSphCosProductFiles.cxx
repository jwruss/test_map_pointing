/*
 * makeSphCosProductFiles.cxx

 *  Created on: Oct 03, 2020
 *      Author: John Russell
 *
 *  Produces files with histograms of spherical cosine products between baselines
...
void makeSphCosProduct(TFile * outFile, AnitaPol::AnitaPol_t pol, int ant1, int ant2) {
	...
	TH2F antSphCosProduct(TString::Format("%s_%d_%d", pol == AnitaPol::kHorizontal ? "H" : "V", ant1, ant2), TString::Format("%s_%d_%d", pol == AnitaPol::kHorizontal ? "H" : "V", ant1, ant2), NPhi, minPhi, maxPhi, NNegTheta, minNegTheta, maxNegTheta);
...
int main(int argc, char * argv[]) {
	...
	TFile coarseFile("sphCosProductCoarse.root", "recreate");

	for (int i = 0; i < NUM_SEAVEYS; ++i) {

		for (int j = i; j < NUM_SEAVEYS; ++j) {

			makeSphCosProduct(& coarseFile, AnitaPol::kHorizontal, i, j);
			makeSphCosProduct(& coarseFile, AnitaPol::kVertical, i, j);
		}
	...
	TFile fineFile("sphCosProductFine.root", "recreate");

	for (int i = 0; i < NUM_SEAVEYS; ++i) {

		for (int j = i; j < NUM_SEAVEYS; ++j) {

			makeSphCosProduct(& fineFile, AnitaPol::kHorizontal, i, j);
			makeSphCosProduct(& fineFile, AnitaPol::kVertical, i, j);
		}
	}
